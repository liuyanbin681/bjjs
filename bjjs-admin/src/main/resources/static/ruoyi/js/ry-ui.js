...
(function ($) {
    $.extend({
    	...
    	table: {
			...
			// 图片预览
			imgView: function (value, path, target) {
				var _path = $.common.isEmpty(path) ? '/profile/avatar' : path;
				// blank or self
				var _target = $.common.isEmpty(target) ? 'self' : target;
				if ($.common.isNotEmpty(value)) {
					return $.common.sprintf("<img class='img-circle img-xs' data-target='%s' src='%s/%s'/>", _target, _path, value);
        ...
        operate: {
			...
			// 解除冻车
			Relieve: function(id) {
				$.modal.confirm("确定要解除冻车吗？", function() {
					var url = $.common.isEmpty(id) ? $.table._option.relieveUrl : $.table._option.relieveUrl.replace("{id}", id);
					if($.table._option.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
						var data = { "ids": id };
